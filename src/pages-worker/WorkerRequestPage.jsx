...
const WorkerTaskRequestsPage = () => {
    ...
    const timer = setTimeout(() => {
      ...
      ws.onmessage = async (event) => {
        try {
          ...
          // ── Re-enrich task when offer is updated ──
  ...
  // ── Register FCM token for this worker ────────────────────────────────────────
useEffect(() => {
  if (!workerId) return;
  const registerToken = async () => {
    try {
      ...
      if (!token) {
        console.warn("[FCM] No token received — user may have denied permission");
        return;
      }

      console.log("[FCM] Worker token:", token);

      // Save token to backend against this workerId
      await fetch("http://127.0.0.1:8000/api/notifications/save-token", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId: workerId, token }),
      });
      console.log("[FCM] Token saved for worker ✓");
    } catch (err) {
...
// ── Foreground FCM listener (tab open but not focused) ────────────────────────
useEffect(() => {
  if (!workerId) return;
  let unsubscribe = () => {};
  const setup = async () => {
    try {
      ...
      unsubscribe = onMessage(messaging, (payload) => {
        ...
        // If it's a new task, re-fetch so the card appears immediately
        ...
        // If it's a status update, patch the task in state
  ...
  const formatCurrency = (amount) => !amount ? "Rs. 0" : `Rs. ${Number(amount).toLocaleString("en-IN")}`;
  ...
  const StatusBadge = ({ status }) => {
    ...
    const Icon = cfg.Icon;
    return (
      <span style={{ display:"inline-flex", alignItems:"center", gap:5, padding:"4px 12px", borderRadius:999, fontSize:12, fontWeight:600, background:cfg.bg, color:cfg.color, border:`1px solid ${cfg.border}` }}>
  ...
  const OfferPendingBanner = () => (
    <div style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 14px", background:"#fffbeb", border:"1px solid #fcd34d", borderRadius:10, fontSize:13, color:"#92400e", marginBottom:12 }}>
      <AlertCircle size={15} color="#f59e0b" style={{ flexShrink:0 }}/>
      <span>Waiting for offer discussion — <strong>estimated hours</strong> and <strong>total cost</strong> must be agreed before accepting.</span>
    </div>
  );
          ...
          {TABS.map(tab => {
            ...
            return (
              <button key={tab} onClick={() => setActiveTab(tab)}
                style={{ padding:"8px 16px", border:"none", background:"none", cursor:"pointer", fontSize:13, fontWeight:isActive?700:500, color:isActive?"#111827":"#6b7280", borderBottom:isActive?"2px solid #f59e0b":"2px solid transparent", marginBottom:-1.5, borderRadius:"6px 6px 0 0", transition:"all 0.15s", display:"flex", alignItems:"center", gap:6 }}
              >
                {tab.charAt(0).toUpperCase() + tab.slice(1).replace("_"," ")}
                {count > 0 && tab !== "all" && (
                  <span style={{ fontSize:10, fontWeight:700, padding:"1px 6px", borderRadius:999, background:isActive?"#f59e0b":"#e5e7eb", color:isActive?"white":"#6b7280" }}>
                    {count}
                  </span>
                )}
              </button>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
          {filteredRequests.length === 0 ? (
            <div style={{ padding:"60px 40px", textAlign:"center", border:"1px solid #e5e7eb", borderRadius:14, background:"white" }}>
              ...
              <h3 style={{ color:"#111827", fontSize:18, fontWeight:600, margin:"0 0 8px" }}>No {activeTab} requests found</h3>
            ...
            filteredRequests.map((request) => {
              ...
              return (
                <div key={rid}
                  style={{ display:"grid", gridTemplateColumns:"220px 1fr 180px", gap:0, padding:"20px 24px", alignItems:"start", border:"1px solid #e5e7eb", borderRadius:14, background:"white", cursor:"pointer", transition:"background 0.15s, border-color 0.15s" }}
                ...
                >
                  ...
                  <div style={{ paddingRight:24, borderRight:"1px solid #f3f4f6" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8 }}>
                      ...
                      <div>
                        <div style={{ fontWeight:700, fontSize:15, color:"#111827" }}>{request.customerName || "Customer"}</div>
                        <div style={{ fontSize:12, color:"#9ca3af" }}>{request.taskName || "General Task"}</div>
                      </div>
                    </div>
                    {request.hourlyRate && (
                  ...
                  <div style={{ padding:"0 24px", borderRight:"1px solid #f3f4f6" }}>
                    ...
                    <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:14 }}>
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:13, color:"#6b7280" }}>
                        ...
                        <span>Scheduled: {formatDate(request.preferredDate)}{request.preferredTime ? ` at ${request.preferredTime}` : ""}</span>
                      </div>
                      <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:13, color:"#6b7280" }}>
                        <Clock size={13} color="#f59e0b"/>
                        <span>Duration: {request.estimatedHours || "Not set"}</span>
                      </div>
                    </div>
                    ...
                    {/* ── Offer not ready warning on card ── */}
                    {isPending && !offerReady && (
                      <div style={{ display:"flex", alignItems:"center", gap:6, padding:"8px 12px", background:"#fffbeb", border:"1px solid #fcd34d", borderRadius:8, fontSize:12, color:"#92400e", marginBottom:12 }}>
                      ...
                      {isPending && (
                        <>
                          <button
                            ...
                            style={{ padding:"7px 16px", background:offerReady?"#059669":"#d1d5db", color:"white", border:"none", borderRadius:8, fontSize:13, fontWeight:600, cursor:(!offerReady||processingAction===rid)?"not-allowed":"pointer", display:"flex", alignItems:"center", gap:5, opacity:processingAction===rid?0.7:1, transition:"background 0.15s" }}
                          >
                  ...
                  <div style={{ paddingLeft:24 }}>
                    ...
                    <div style={{ marginBottom:10 }}>
                      ...
                      <div style={{ fontSize:14, fontWeight:600, color: request.estimatedHours ? "#374151" : "#f59e0b" }}>
                    ...
                    <div style={{ borderTop:"1px dashed #e5e7eb", paddingTop:10 }}>
                      ...
                      <div style={{ fontSize:18, fontWeight:700, color: request.totalCost ? "#f59e0b" : "#9ca3af" }}>
        ...
        return (
          ...
          >
            ...
            >
              ...
              {/* Offer not ready banner in modal */}
              {isPending && !offerReady && <OfferPendingBanner/>}
              ...
              {isPending && (
                <div style={{ display:"flex", gap:10 }}>
                  <button
                    ...
                    title={!offerReady ? "Estimated hours and total cost must be set first" : ""}
                    style={{ flex:1, padding:"12px", background:offerReady?"#059669":"#d1d5db", color:"white", border:"none", borderRadius:10, fontWeight:600, cursor:!offerReady?"not-allowed":"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:6, transition:"background 0.15s" }}
                  >
