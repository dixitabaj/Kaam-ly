...
// ── Small reusable button (matches CustomerTaskPage Btn) ─────────────────────
const Btn = ({ onClick, variant = "default", children, disabled, title }) => {
  const V = {
  ...
  return (
    <button
      ...
      style={{
        ...
        color:disabled?v.color:v.color, background:disabled?"#f3f4f6":v.bg,
...
const WorkerTaskRequestsPage = () => {
  ...
  const handleAcceptRequest = async (requestId) => {
    ...
    if (!isOfferReady(task)) {
      addToast({ color: "#b45309", message: "Cannot accept: estimated hours and total cost must be set first." });
      return;
    }
  ...
  const confirmDecline = async () => {
    ...
    try {
      ...
      const apply = (r) =>
        String(r._id || r.id) === String(decliningRequestId) ? { ...r, status: "declined", declineReason } : r;
      setRequests((prev) => prev.map(apply));
      setSelectedRequest((prev) => (prev ? apply(prev) : prev));
  ...
  const taskCounts = requests.reduce((acc, r) => {
    ...
    <div style={{ minHeight:"100vh", background:"#f9f6ef", fontFamily:'"DM Sans",-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif', color:"#1c1008" }}>
      ...
      <main style={{ maxWidth:"1280px", margin:"0 auto", padding:"2rem 1.5rem" }}>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:"1.25rem", marginBottom:"1.75rem" }}>
          <div>
            <h1 style={{ fontSize:"26px", fontWeight:"900", color:"#1c1008", margin:"0 0 4px", letterSpacing:"-0.02em" }}>Your Task Details</h1>
            <p style={{ color:"#a8a29e", fontSize:"13px", fontWeight:"500", margin:0 }}>Manage incoming requests from customers</p>
          </div>
          <div style={{ display:"flex", gap:"10px", flexWrap:"wrap" }}>
            {[
              ...
              <div key={label} style={{ textAlign:"center", background:"white", border:"1px solid #ede8df", borderRadius:"14px", padding:"10px 18px", minWidth:"74px", boxShadow:"0 1px 4px rgba(0,0,0,0.04)" }}>
                <div style={{ fontSize:"18px", fontWeight:"900", color:"#1c1008" }}>{value}</div>
                <div style={{ fontSize:"10px", color:"#a8a29e", fontWeight:"700", marginTop:"3px", textTransform:"uppercase", letterSpacing:"0.04em", display:"flex", alignItems:"center", justifyContent:"center", gap:"4px" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:"14px" }}>
          {filteredRequests.length === 0 ? (
            ...
            filteredRequests.map((request) => {
              ...
              const [hovered, setHovered] = [false, () => {}]; // placeholder; use local component below
            ...
            >
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"1rem", marginBottom:"1.25rem" }}>
                {[
                  ...
                  <div key={label}>
                    <div style={{ fontSize:"10px", color:"#a8a29e", textTransform:"uppercase", letterSpacing:"0.06em", marginBottom:"4px", fontWeight:"700" }}>{label}</div>
                    <div style={{ fontSize:"14px", color:"#1c1008", fontWeight:"600" }}>{value}</div>
                  </div>
                ))}
              </div>
              ...
              {isPending && (
                <div style={{ display:"flex", gap:"8px" }}>
                  <button
                    ...
                    style={{ flex:1, padding:"11px", background:offerReady?"linear-gradient(135deg,#059669,#047857)":"#e5e7eb", color:"white", border:"none", borderRadius:"10px", fontWeight:"700", cursor:!offerReady?"not-allowed":"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:"6px", fontSize:"13px", transition:"opacity 0.15s" }}
                  >
      ...
      {showDeclineModal && (
        ...
        >
          ...
          >
            ...
            />
            <div style={{ display:"flex", gap:"8px" }}>
              <button onClick={() => setShowDeclineModal(false)} style={{ flex:1, padding:"10px", borderRadius:"10px", border:"1.5px solid #e8dfd0", background:"white", fontSize:"13px", fontWeight:"700", cursor:"pointer", color:"#78716c" }}>
                Cancel
              </button>
              <button onClick={confirmDecline} style={{ flex:2, padding:"10px", borderRadius:"10px", border:"none", background:"#ef4444", fontSize:"13px", fontWeight:"700", cursor:"pointer", color:"white" }}>
                Confirm Decline
              </button>
            </div>
          </div>
...
// ── Request Card (extracted to support hooks cleanly) ─────────────────────────
const RequestCard = ({
  ...
  return (
    ...
    >
      {/* LEFT: Customer */}
      <div style={{ paddingRight:"24px", borderRight:"1px solid #f0ebe2" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"10px" }}>
          <div style={{ width:"46px", height:"46px", borderRadius:"50%", background:"linear-gradient(135deg,#f6a623,#e8890c)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"18px", fontWeight:"800", color:"white", flexShrink:0, boxShadow:"0 2px 8px rgba(246,166,35,0.3)" }}>
            {request.customerName?.charAt(0) || <User size={16} color="white"/>}
          </div>
          <div style={{ minWidth:0 }}>
            <div style={{ fontWeight:"700", fontSize:"14px", color:"#1c1008", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{request.customerName || "Customer"}</div>
            <div style={{ fontSize:"11px", color:"#a8a29e", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{request.taskName || "General Task"}</div>
          </div>
        </div>
        {request.hourlyRate && (
          <div style={{ fontSize:"11px", color:"#a8a29e", marginBottom:"10px" }}>
      ...
      {/* MIDDLE: Details */}
      <div style={{ padding:"0 24px", borderRight:"1px solid #f0ebe2" }}>
        <div style={{ fontSize:"14px", color:"#1c1008", marginBottom:"12px", fontWeight:"600" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:"7px", marginBottom:"12px" }}>
          <div style={{ display:"flex", alignItems:"flex-start", gap:"7px" }}>
            ...
            <span style={{ fontSize:"13px", color:"#57534e", lineHeight:"1.4" }}>{request.address || "Not specified"}</span>
          </div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"7px" }}>
            ...
            <span style={{ fontSize:"13px", color:"#57534e" }}>Duration: {request.estimatedHours || "Not set"}</span>
          </div>
        </div>
    ...
    <Btn onClick={e => { e.stopPropagation(); handleViewDetails(request); }} variant="default">
      ...
      <div style={{ paddingLeft:"24px", display:"flex", flexDirection:"column" }}>
        <div style={{ marginBottom:"12px" }}>
          <div style={{ fontSize:"10px", color:"#9ca3af", textTransform:"uppercase", letterSpacing:"0.6px", marginBottom:"2px", fontWeight:"700" }}>Estimated price</div>
          <div style={{ fontSize:"15px", fontWeight:"700", color:"#1c1008" }}>{formatCurrency(request.workerEarnings || request.totalCost)}</div>
        </div>
        <div style={{ marginBottom:"12px" }}>
          <div style={{ fontSize:"10px", color:"#9ca3af", textTransform:"uppercase", letterSpacing:"0.6px", marginBottom:"2px", fontWeight:"700" }}>Estimated hours</div>
          <div style={{ fontSize:"13px", fontWeight:"600", color:request.estimatedHours ? "#374151" : "#f6a623" }}>
            {request.estimatedHours || "Not set yet"}
          </div>
        </div>
        <div style={{ borderTop:"1px dashed #e8dfd0", paddingTop:"12px" }}>
          <div style={{ fontSize:"10px", color:"#9ca3af", textTransform:"uppercase", letterSpacing:"0.6px", marginBottom:"2px", fontWeight:"700" }}>Total cost</div>
          <div style={{ fontSize:"17px", fontWeight:"900", color:request.totalCost ? "#f6a623" : "#c4bab0" }}>
