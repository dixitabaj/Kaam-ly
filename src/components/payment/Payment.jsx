...
function StepPay({ task, onPaid }) {
  ...
  return (
    <div style={S.panel}>
      ...
      <div style={S.infoRow}>
        <span style={S.infoKey}>Task</span>
        <span style={S.infoVal}>{task.taskName}</span>
      </div>
      <div style={S.infoRow}>
        <span style={S.infoKey}>Service Date</span>
        <span style={S.infoVal}>{task.serviceDate ? new Date(task.serviceDate).toLocaleDateString() : "—"}</span>
      </div>
      <div style={S.infoRow}>
        <span style={S.infoKey}>Address</span>
        <span style={S.infoVal}>{task.address || "—"}</span>
      </div>
      ...
      <div style={{ background: "#f0fdf4", borderRadius: 10, padding: "12px 14px", marginBottom: 18, display: "flex", gap: 10, alignItems: "flex-start" }}>
        ...
        <p style={{ margin: 0, fontSize: 12, color: "#166534", lineHeight: 1.5 }}>
          Funds are held securely in escrow until you confirm the task is complete. You stay in control.
        </p>
      </div>
      ...
      <button onClick={handlePay} disabled={loading} style={{ ...S.btn, ...S.btnGreen }}>
        {loading
          ? <><span style={S.spin} /> Processing…</>
          : <><EsewaIcon /> Pay with eSewa</>
        }
      </button>
    </div>
  );
    ...
    <div style={S.panel}>
      <div style={S.statusBox}>
        ...
        <div>
          ...
          <p style={S.statusSub}>
            NPR {task.totalCost?.toLocaleString()} is safely held in escrow.
...
// ─── STEP 3: Customer Confirms Complete ───────────────────────
  ...
  return (
    <div style={S.panel}>
      ...
      <div style={{ background: "linear-gradient(135deg,#ecfdf5,#d1fae5)", borderRadius: 14, padding: "18px 20px", marginBottom: 20, display: "flex", gap: 14, alignItems: "center", border: "1px solid #a7f3d0" }}>
        ...
        <div>
          <p style={{ margin: 0, fontWeight: 800, color: "#065f46", fontSize: 18 }}>
            NPR {task.totalCost?.toLocaleString()}
          </p>
          <p style={{ margin: "2px 0 0", fontSize: 12, color: "#047857" }}>
            Confirming releases funds to the worker immediately.
          </p>
        </div>
      </div>
      ...
      <label style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 20, cursor: "pointer", padding: "14px 16px", background: "#f9fafb", borderRadius: 10, border: confirmed ? "1.5px solid #10b981" : "1.5px solid #e5e7eb" }}>
        ...
        <span style={{ fontSize: 13, color: "#374151", lineHeight: 1.5 }}>
          I confirm the task has been completed to my full satisfaction and approve releasing payment to the worker.
        </span>
      </label>
    ...
    <div style={S.panel}>
      <div style={S.statusBox}>
        ...
        <div>
          ...
          <p style={S.statusSub}>
            NPR {task.totalCost?.toLocaleString()} is held securely. Complete the task
            and ask the customer to confirm to receive payment.
          </p>
        </div>
      </div>
      <div style={S.timeline}>
...
// ─── STEP 5: Worker — Waiting for Payment ─────────────────────
    ...
    <div style={S.panel}>
      <div style={S.statusBox}>
        ...
        <div>
          ...
          <p style={S.statusSub}>The customer hasn't paid yet. You'll be notified once payment is confirmed and you can begin.</p>
        </div>
      </div>
    </div>
  );
...
function Step({ label, done, active }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <span style={{ fontSize: 12, fontWeight: done ? 600 : active ? 700 : 400, color: done ? "#065f46" : active ? "#5b21b6" : "#9ca3af" }}>
        {label}
      </span>
    </div>
  );
...
export default function PaymentFlow() {
  const { taskId, userId, role } = useParams();
  ...
  const [error, setError] = useState("");
  const [successMsg, setSuccessMsg] = useState(paymentSuccess ? "Payment verified! Funds are now held in escrow." : "");
  ...
  if (loading) return (
    <div style={S.page}>
      <div style={{ ...S.panel, textAlign: "center", padding: 60 }}>
        <div style={{ ...S.spin, width: 36, height: 36, margin: "0 auto 16px", borderColor: "#e5e7eb", borderTopColor: "#10b981", borderWidth: 3 }} />
  ...
  return (
    <div style={S.page}>
      ...
      {/* ── Header ── */}
      <div style={{ animation: "fadeUp .4s ease both", animationDelay: "0s" }}>
        <div style={S.header}>
          <div>
            ...
            <h1 style={{ margin: "4px 0 0", fontSize: 22, fontWeight: 800, color: "#111" }}>
              {task.taskName || "Task Payment"}
            </h1>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#6b7280" }}>{task.address}</p>
          </div>
      ...
      {/* ── Success Banner ── */}
      {successMsg && (
        <div style={{ animation: "fadeUp .4s ease both", animationDelay: "0.05s" }}>
          <div style={{ background: "#ecfdf5", border: "1.5px solid #6ee7b7", borderRadius: 14, padding: "14px 18px", display: "flex", gap: 10, alignItems: "center" }}>
            <span style={{ fontSize: 20 }}>🎉</span>
            <p style={{ margin: 0, fontWeight: 700, color: "#065f46", fontSize: 14 }}>{successMsg}</p>
          </div>
        </div>
      )}

      {/* ── Flow ── */}
      <div style={{ animation: "fadeUp .4s ease both", animationDelay: "0.1s" }}>
        {role === "customer" && (
          <>
            {!isPaid && !isCompleted && (
              <StepPay task={task} onPaid={() => refresh({ payment_status: "paid", escrow_status: "held" })} />
            )}
