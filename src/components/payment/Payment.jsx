  ...
  return (
    <>
      ...
      <div style={{ background: "#f0fdf4", borderRadius: 10, padding: "12px 14px", marginBottom: 18, display: "flex", gap: 10, alignItems: "flex-start" }}>
        ...
        <p style={{ margin: 0, fontSize: 12, color: "#166534", lineHeight: 1.5 }}>Funds are held securely in escrow until you confirm the task is complete. You stay in control.</p>
      </div>
...
function StepConfirmComplete({ task, userId, onComplete }) {
  ...
  const handleComplete = async () => {
    ...
    setLoading(true); setError("");
    try {
      const res = await fetch(`${API}/task/${task._id}/complete`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ user_id: userId }) });
      const data = await res.json();
  ...
  return (
    <>
      ...
      <label style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 20, cursor: "pointer", padding: "14px 16px", background: "#f9fafb", borderRadius: 10, border: confirmed ? "1.5px solid #10b981" : "1.5px solid #e5e7eb" }}>
        ...
        <span style={{ fontSize: 13, color: "#374151", lineHeight: 1.5 }}>I confirm the task has been completed to my full satisfaction and approve releasing payment to the worker.</span>
      </label>
      {error && <p style={S.error}>{error}</p>}
    ...
    <>
      <div style={S.statusBox}>
        ...
        <div>
          ...
          <p style={S.statusSub}>NPR {task.totalCost?.toLocaleString()} is held securely. Complete the task and ask the customer to confirm to receive payment.</p>
        </div>
      </div>
      <div style={S.timeline}>
  ...
  return (
    <div style={{ textAlign: "center", padding: "32px 0" }}>
      ...
      <p style={{ margin: "8px 0 0", fontSize: 14, color: "#6b7280" }}>
        {role === "worker" ? `NPR ${(task.worker_payout || task.totalCost)?.toLocaleString()} has been released to your account.` : "Thank you! Payment has been released to the worker."}
      </p>
    </div>
  );
}
...
// ─── MAIN COMPONENT ───────────────────────────────────────────
// Works in two modes:
//   1. Standalone route  → reads taskId/userId/role from useParams()
//   2. Inline modal      → receives taskIdProp/userIdProp/roleProp + onClose prop
  ...
  return (
    <>
      ...
      >
        {/* ── Modal card ── */}
        <div
          style={{ background:"#fff", borderRadius:24, width:"100%", maxWidth:480, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 32px 80px rgba(0,0,0,.3)", animation:"fadeUp .35s cubic-bezier(0.16,1,0.3,1) both", display:"flex", flexDirection:"column" }}
        ...
        >
          ...
          <div style={{ padding:"20px 24px 16px", borderBottom:"1px solid #f1f1f1", display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
            ...
            <div style={{ display:"flex", alignItems:"flex-start", gap:8, flexShrink:0, marginLeft:12 }}>
              ...
              {isModal && (
                <button onClick={onClose} style={{ width:30, height:30, borderRadius:"50%", background:"#f3f4f6", border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, color:"#6b7280", fontSize:18, fontWeight:700, marginTop:2 }}>
                  ×
                </button>
              )}
            </div>
          ...
          <div style={{ padding:"20px 24px 24px" }}>
            ...
            {!loading && task && (
              <>
                {role === "customer" && (
                  <>
                    {!isPaid && !isCompleted && <StepPay task={task} onPaid={() => refresh({ payment_status:"paid", escrow_status:"held" })} />}
...
const S = {
  ...
  btn:        { width:"100%", padding:"14px 20px", border:"none", borderRadius:12, fontSize:14, fontWeight:700, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8, fontFamily:"'DM Sans', sans-serif", transition:"opacity .2s, transform .15s" },
